...
if ( ! function_exists ( 'sktm_maintenance_install_cart' ) ) {
  function sktm_maintenance_install_cart(){
    ...
    $sktm_maintenance_excludepage = $wpdb->prefix .'skt_maintenance_excludepage';
     ...
     dbDelta( $sqlpage );
     if($sqlpage){
      $result = $wpdb->get_results("SELECT * from $sktm_maintenance_excludepage");
      if(count($result) == 0){
      $insert_query = $wpdb->query("INSERT INTO $sktm_maintenance_excludepage (ID, exclude_page_id, exclude_post_id) VALUES
          (1,'0','0');");
      }
    }
  }
...
function sktm_maintenance_hex2rgb($hex) {
  ...
  function sktm_maintenance_mode_css() {
    ...
    if( $pagenow !== 'wp-login.php' && !is_user_logged_in() && $maintenancemode=="1") {
      ...
      $exclude_page_id = "";
      $explode_post_id = "";
      $select_maintenancemode = $wpdb->get_row("SELECT* FROM $sktm_maintenance_excludepage");
    ...
    <html>
      <head>
        ...
        <link rel="stylesheet" type="text/css" href="<?php echo esc_url(SKTM_MAINTENANCE_URI.'/css/style.css'); ?>">
        <link rel="stylesheet" type="text/css" href="https://fonts.googleapis.com/css?family=<?php echo esc_attr($fontfamily); ?>">
        <style type="text/css">
          body{<?php if($backgroundimage !=''){ ?>background-image: url(<?php echo esc_url($backgroundimage); ?>);<?php } ?> background-color: <?php echo $background_bodycss ?>; font-family: <?php echo esc_attr($fontfamily);?>;}
          .skt-maintenance-overlay{background-color: rgba(<?php echo sktm_maintenance_hex2rgb($background_overlay_bodycss); ?>, <?php echo esc_attr($background_overlay_opc_bodycss); ?>);}
      ...
      <body>
        <a class="skt-maintenance-login" href="<?php echo wp_login_url();?>"><img src="<?php echo esc_url(SKTM_MAINTENANCE_URI.'/images/lock-icon.png'); ?>"></a>
